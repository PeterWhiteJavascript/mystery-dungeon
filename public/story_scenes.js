    ...
    Q.playMusic(data.music+".mp3",function(){ 
        switch(data.scene){
            case "Prologue_00_end":
                var stage = Q.stage(1);
                var doneDisappearing = Q("Participant",1).items.length;
                var timer;
                var interaction = [
                    ...
                    {asset:"Professor_Story_Idle.png",pos:"right",text:["Let's get back to the village now!","We must tell the chief what has happened here.",
                        {obj:window,func:"setTimeout",props:function(){
                            //Does some generic code, and also the function that is passed in
                            Q.allToStorySprite(function(obj){
                                obj.p.onArrival = [{func:"disappear"}];
                                obj.startAutoMove([14,0]);
                                obj.on("disappeared",obj,function(){doneDisappearing--;if(doneDisappearing<=0){Q.goToNextScene();clearTimeout(timer);}});
                            });
                                    ...
                                    func:function(dir){
                                        ...
                                        var interaction = [
                                            {asset:"Professor_Story_Idle.png",pos:"right",text:["You have done well enough to be called my aprentice!","It is getting dark, let us depart for home.",
                                                    ...
                                                    {obj:prof,func:"startPresetAutoMove",props:[[13,13],[14,13]]},
...
Q.makeScene = function(sceneName,path,levelName,users,parts,objects,callback){
    Q.scene(sceneName,function(stage){
        ...
        for(var i=0;i<objs.length;i++){
            ...
            var part = stage.insert(new Q.Participant({
            ...
            if(Q._isNumber(part.p.playerId)||part.p.playerId[0]==="a"){part.p.ally="Player";}else{part.p.ally="Enemy";};
            
        }
        //Create the user sprites which allow the user to control things
        for(var i=0;i<users.length;i++){
            var user = stage.insert(new Q.User({playerId:users[i].playerId,file:users[i].file,ready:users[i].ready,name:users[i].name}));
            user.p.player = Q("Participant",1).items.filter(function(obj){
                return obj.p.playerId===user.p.playerId;
            })[0];
            if(user.p.playerId===Q.state.get("playerConnection").id){
                Q.state.get("playerConnection").user = user;
            }
        }
        //Put all objects in the stage (things that do not have a turn but can still be interacted with in some way)
        if(Q._isArray(objects)){
            for(var i=0;i<objects.length;i++){
                Q.setUpObject(stage,objects[i].className,objects[i]);
            } 
        } else if(Q._isObject(objects)){
            var keys = Object.keys(objects);
            for(var i=0;i<keys.length;i++){
                for(var j=0;j<objects[keys[i]].length;j++){
                    Q.setUpObject(stage,keys[i],{loc:objects[keys[i]][j],playerId:i+j+"o"});
                }
            }
        }
